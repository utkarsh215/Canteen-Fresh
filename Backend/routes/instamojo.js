...
router.post("/create-order",(req,res)=>{
    ...
    const options = {
        ...
        redirect_url: `http://localhost:3000/api/check-payment-status?token=${req.body.token}`
        })
      };
              ...
              fetch(`https://test.instamojo.com/v2/payments/${req.query.payment_id}/`, options)
                ...
                .then(response =>{
                    if(response.status === true){
                        ...
                        res.redirect(`http://localhost:5173/PaymentSuccess?status=${response.status}`)
                    }
                    else{
                        res.redirect(`http://localhost:5173/PaymentFailure?status=${response.status}`)
                    }
